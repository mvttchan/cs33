...
{
	uint64_t asize;      /* adjusted block size */
	uint64_t extendsize; /* amount to extend heap if no fit */
    char *bp;      
...
/* $end mmplace-proto */
{
	uint64_t csize = GET_SIZE(HDRP(bp));
...
static void *coalesce(void *bp) 
{
	uint64_t prev_alloc = GET_ALLOC(FTRP(PREV_BLKP(bp)));
	uint64_t next_alloc = GET_ALLOC(HDRP(NEXT_BLKP(bp)));
	uint64_t size = GET_SIZE(HDRP(bp));
